...
def admin_command_add_command():
    ...
    if judge_str(number) == "输入中不能包含空格,请重新输入":
        admin_command_add_v.set("输入中不能包含空格,请重新输入")
    elif judge_str(number) == "输入中含有空字符，请重新输入":
        admin_command_add_v.set("输入中含有空字符，请重新输入")
            ...
            admin_command_add_v.set("请输入大于0纯数字")
    else:
...
def main_frame_admin_management_command_add_windows(root):
    ...
    main_frame_admin_management_command_add_l6 = Label(main_frame_admin_management_command_add, text='        ').grid(row=0, column=2, rowspan=6)
    ...
    main_frame_admin_management_command_add_b1 = Button(main_frame_admin_management_command_add, text='保存', command=admin_command_add_command, font=('微软雅黑', 11)).grid(row=0, column=3, rowspan=5)

    main_frame_admin_management_command_add_l3 = Label(main_frame_admin_management_command_add, textvariable=admin_management_add_v).grid(row=5, column=0)
...
# 用户-推荐-列表-根
def user_recommand_show():
    destroy_root()
    global root_user_recommand_show
    root_user_recommand_show = Tk()
    add_root_to_rootlist(root_user_recommand_show)
    root_user_recommand_show.geometry("%dx%d+%d+%d" % (800, 400, (root_user_recommand_show.winfo_screenwidth() - 800) / 2, (root_user_recommand_show.winfo_screenheight() - 400) / 2))
    root_user_recommand_show.title('用户-推荐')

    user_recommand()
    user_recommand_show_windows(root_user_recommand_show, recommand_list)
...
def user_recommand_input_windows(root):
    ...
    main_frame_user_recommand_input_b1 = Button(main_frame_user_recommand_input, text='确定', command=user_recommand_command).grid(row=4, column=4, rowspan=4)
    main_frame_user_recommand_input_b2 = Button(main_frame_user_recommand_input, text='一键推荐', command=user_recommand_direct).grid(row=0, column=4)
